...
function doExport() {
	...
	while(item = Zotero.nextItem()) {
		...
		var shortFormat = Zotero.getOption("Short Format");
		...
		if(shortFormat) {
			doubleNewline = true;
			if(type == "article") {
				if(Zotero.getOption("useJournalAbbreviation")) { if(item.journalAbbreviation) { writeField("journal", item.journalAbbreviation); } }
				else { if(item.publicationTitle) { writeField("journal", item.publicationTitle); } }

				if(item.volume) { writeField("volume", item.volume); }
				if(item.issue) { writeField("number", item.issue); }
				if(item.pages) { writeField("pages", item.pages.replace(/[-\u2012-\u2015\u2053]+/g, "--")); }
			}

			else if(type == "inproceedings") {
				if(item.publicationTitle) { writeField("booktitle", item.publicationTitle); }
				if(item.volume) { writeField("volume", item.volume); }
				if(item.issue) { writeField("number", item.issue); }
				if(item.pages) { writeField("pages", item.pages.replace(/[-\u2012-\u2015\u2053]+/g, "--")); }
			}
				...
				if(item.publisher) { writeField("publisher", item.publisher); }
				if(item.publicationTitle) { writeField("booktitle", item.publicationTitle); }
				if(item.volume) { writeField("volume", item.volume); }
				if(item.issue) { writeField("number", item.issue); }
				if(item.pages) { writeField("pages", item.pages.replace(/[-\u2012-\u2015\u2053]+/g, "--")); }
				...
				if (item.university) { writeField("school", item.university); }
			}

			else if(type == "misc") {
				if(item.url) { writeField("howpublished", item.url); }
			}
